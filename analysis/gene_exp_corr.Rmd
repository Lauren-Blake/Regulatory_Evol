...
plot.pca.true <- plot.pca.true + theme(legend.position = "bottom") + scale_shape_discrete(name = c(""), breaks = c("chimpanzee", "human", "rhesus macaque"), labels = c("Chimpanzee", "Human", "Rhesus macaque")) 
...
plot.pca.methyl <- plot.pca.methyl + theme(legend.position = "bottom") + scale_shape_discrete(name = c(""), breaks = c("chimpanzee", "human", "rhesus macaque"), labels = c("Chimpanzee", "Human", "Rh. macaque")) 
...
save_plot("/Users/laurenblake/Dropbox/Tissue_paper/Supplement/Figures/fig1.pdf", make_fig1,
...
methyl_switch <- methyl
methyl_switch[,38] <- methyl[,42]
methyl_switch[,42] <- methyl[,38]

methyl_switch[,39] <- methyl[,43]
methyl_switch[,43] <- methyl[,39]
# Calculate the correlations for the RNA-seq and methylation correct individual comparison
cor_real <- c()

for (i in 1:47){
calc_cor <- cor(express[,i], methyl_switch[,i])
cor_real <- c(cor_real, calc_cor)
}
...
ch1_cor <- c()
for (i in ch1){
calc_cor <- cor(express[,expression_row], methyl_switch[,i])
ch1_cor <- c(ch1_cor, calc_cor)
  }
return(ch1_cor)
}
...
rh2_cor_func <- make_correlations(c(32, 40, 44), 36)
rh3_cor_func <- make_correlations(c(32, 36, 44), 40)
rh4_cor_func <- make_correlations(c(32, 36, 40), 44) #102

# Rhesus kidney
rk1_cor_func <- make_correlations(c(37, 41, 45), 33)
rk2_cor_func <- make_correlations(c(33, 41, 45), 37)
rk3_cor_func <- make_correlations(c(33, 37, 45), 41)
rk4_cor_func <- make_correlations(c(33, 37, 41), 45) #114

cor(express[,41], methyl[,41])
...
which(grepl(min(hcr_cor), hcr_cor))

summary(cor_real)
summary(hcr_cor)

boxplot(cor_real, hcr_cor)

boxplot(cor_real, hcr_cor, xaxt = "n", xlab = "RNA-seq and BS-seq from same individuals", ylab = "Correlation")
axis(1, at = 1:2, labels = c("Matched", "Non-matched"))
```

# Look at human/chimpanzee data only
