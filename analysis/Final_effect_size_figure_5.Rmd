...
h_spec <- ggplot(combine_datab, aes(combine_datab$Effect, combine_datab$SE)) + geom_point(color = c(combine_datab$ind_sigb)) + ylab(" ") + xlab("Effect size difference") + ylim(0,5.5) + xlim(-8, 8)
```
...
top_row <- plot_grid(h_cons, h_spec, h_non, 
                        labels = c('5A.', '5B.', '5C.'), nrow = 1)

bottom_row <- plot_grid(plot_intertissue, labels = c('5D.'))

plot_fig5 <- plot_grid(top_row, bottom_row, ncol = 1, nrow = 2, rel_heights = c(1,1.5))

#save_plot("/Users/laurenblake/Dropbox/Tissue_paper/Draft_4/Figure_5.png", plot_fig5,
