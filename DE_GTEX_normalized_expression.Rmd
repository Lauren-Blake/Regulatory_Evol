...
```{r}
# Make labels
heart_label <- array("heart", dim = c(1,103))
lung_label <- array("lung", dim = c(1,103))
...
PC2 <- four_cov$C2
PC3 <- four_cov$C3
IC1 <- four_cov$InferredCov1
IC2 <- four_cov$InferredCov2
IC3 <- four_cov$InferredCov3
...
dge_shared <- cbind(common_ind_lung_counts_platform, common_ind_lung_counts_platform)

dge_in_cutoff <- DGEList(counts=as.matrix(dge_shared), genes=rownames(dge_shared), group = as.character(labels))
dge_shared <- calcNormFactors(dge_in_cutoff)

# Voom with individual as a random variable

cpm.voom.cyclic <- voom(dge_shared, design, normalize.method="cyclicloess", plot=T)

all_ind <- rbind(colnames(common_ind_heart_counts_platform), colnames(common_ind_lung_counts_platform))

corfit <- duplicateCorrelation(cpm.voom.cyclic, design, block = all_ind)
#corfit$consensus.correlation
#corfit.consensus <-  0.06519918
...
heart_lung_all <- topTable(fit.cyclic.norm, coef=2, adjust="BH", number=Inf, sort.by="none")
dim(heart_lung_all[which(heart_lung_all$adj.P.Val < 0.05), ]) 
