...
## Aug 20, 2015
# - Run on spudhead
# - Launches overlap_repeats_DMRs.pl for all DMRs, and all the random DMRs files of same length /CpG density
...
foreach my $file (@files){
  ...
  # launch script overlap_repeats_DMRs.pl: already done by launch_overlap_repeats_DMRs.pl
  ...
  for (my $i=1; $i <= 100; $i++) {
    system("echo \"perl overlap_repeats_DMRs.pl ./DMRs/$DMRtype/randomized_control_CpG_density/$randomDMRfile$i.bed\" | qsub -l h_vmem=4g -v PATH -cwd -N $randomDMRfile$i -o ./DMRs/$DMRtype/randomized_control_CpG_density/$randomDMRfile$i.out -e ./DMRs/$DMRtype/randomized_control_CpG_density/$randomDMRfile$i.err");
  }
  #last; # testing
}
