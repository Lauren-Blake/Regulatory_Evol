...
## Find individual DMRs for Rhesus
## There was a swapping between samples labels
## R3Li is R2Li, R3Lu is R2Lu and conversely
...
for (species in c("Rhesus")){  
  ...
  ## Mix up some columns to fix the R2/R3 samples swapping issue
  ## R3Li -> R2Li, R3Lu -> R2Lu, R2Li -> R3Li, R2Lu -> R3Lu 
  colNames <- colnames(M)
  M <- M[, c(1:6, 11:12, 9:10, 7:8, 13:16)]
  colnames(M) <- colNames
  Cov <- Cov[, c(1:6, 11:12, 9:10, 7:8, 13:16)]
  colnames(Cov) <- colNames
  coef <- coef[, c(1:6, 11:12, 9:10, 7:8, 13:16)]
  colnames(coef) <- colNames
  cat("Data columns swapped!\n")
