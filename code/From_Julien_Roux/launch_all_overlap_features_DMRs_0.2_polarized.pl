...
## Oct 30 2014
# - Run on spudhead
# - Launches overlap_features_DMRs_0.2.pl for all DMRs, and all the random DMRs files of same length
...
Usage:
perl launch_all_overlap_features_DMRs_0.2_polarized.pl <DMR type>
USAGE
my $DMRtype = $ARGV[0];
...
### my @files = glob("../bsseq/DMRs/$DMRtype/Rhesus_liverSpecific*DMRs.txt");
...
foreach my $file (@files){
  foreach my $direction (('hypo', 'hyper')){
    ...
    # This has been done by script launch_all_overlap_features_DMRs_0.2_control_CpG_density_polarized.pl
    ...
    for (my $i=1; $i <= 100; $i++) {
      # Export BED file
      ## system("sed -n". $positions{$direction}." ./DMRs/$DMRtype/randomized/$baseName\_randomDMRs\_$i.bed > ./DMRs/$DMRtype/randomized/$randomDMRfile$i.bed");
      ## Doesn't work when number of arguments is too big! This should work better:

      open(OUT, ">./DMRs/$DMRtype/randomized/$randomDMRfile$i.bed") or die ("Cannot open BED file\n");
      open(IN, "./DMRs/$DMRtype/randomized/$baseName\_randomDMRs\_$i.bed") or die ("Cannot open DMRs file\n");
      my $j = 1;
      while (defined (my $line = <IN>)) {
        chomp $line;
