...
## Sep 2, 2015
# - Run on spudhead
# - Launches overlap_promoters_enhancers_features_DMRs.pl for all DMRs, and all the random DMRs files of same length (no control for CpG density)
...
foreach my $file (@files){
  ...
  # launch script overlap_promoters_enhancers_features_DMRs.pl: 
  ...
  for (my $i=1; $i <= 100; $i++) {
    system("echo \"perl overlap_promoters_enhancers_features_DMRs.pl ./DMRs/$DMRtype/randomized/$randomDMRfile$i.bed\" | qsub -l h_vmem=4g -v PATH -cwd -N $randomDMRfile$i -o ./DMRs/$DMRtype/randomized/$randomDMRfile$i.out -e ./DMRs/$DMRtype/randomized/$randomDMRfile$i.err");
  }
  #last; # testing
}
